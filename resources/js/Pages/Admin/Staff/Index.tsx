    ...
    return (
        <AdminLayout title="Staff">
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <h1 className="mt-1 font-display text-3xl font-bold">
                        Staff <span className="text-muted-foreground">· {staff.length}</span>
                    </h1>
                </div>
            ...
            {filtered.length === 0 ? (
                <EmptyState
                    icon={Users}
                    title={search ? 'Aucun membre trouvé' : 'Aucun membre du staff'}
                    description={
                        search
                            ? 'Essaie une autre recherche ou vide le filtre.'
                            : 'Ajoute le premier membre du staff pour démarrer.'
                    }
                    action={
                        !search ? (
                            <Button asChild>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-border bg-muted/50 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                                ...
                                <th className="hidden px-4 py-3 text-left md:table-cell">Poste</th>
            ...
            <ConfirmDialog
                ...
                description="Le membre et sa photo seront supprimés définitivement."
                confirmLabel="Supprimer"
                variant="destructive"
                onCancel={() => setToDelete(null)}
                onConfirm={() => {
                    if (!toDelete) return;
                    router.delete(`/staff/${toDelete.id}`, {
            ...
            <ConfirmDialog
                open={confirmBulk}
                title={`Supprimer ${selected.size} membre${selected.size > 1 ? 's' : ''} ?`}
                description="Cette action est irréversible. Les membres du staff et leurs photos seront supprimés définitivement."
                confirmLabel="Supprimer"
                variant="destructive"
    ...
    return (
        ...
        >
            <td className="px-4 py-3">
                <input type="checkbox" checked={isSelected} onChange={onToggle} className="h-4 w-4 cursor-pointer accent-crimson" aria-label={`Sélectionner ${member.first_name} ${member.last_name}`} />
            </td>
            <td className="px-4 py-3">
