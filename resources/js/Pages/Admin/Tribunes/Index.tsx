...
export default function TribunesIndex({ tribunes }: Props) {
    ...
    return (
        <AdminLayout title="Tribunes">
                        ...
                        <tbody>
                            {filtered.map((tribune, i) => (
                                <TribuneRow
                                    ...
                                    tribune={tribune}
                                    index={i}
                                    isSelected={selected.has(tribune.id)}
                                    onToggle={() => toggle(tribune.id)}
            ...
            <ConfirmDialog
                open={confirmBulk}
                title={`Supprimer ${selected.size} tribune${selected.size > 1 ? 's' : ''} ?`}
                description="Cette action est irréversible. Les tribunes et leurs photos seront supprimées définitivement."
                confirmLabel="Supprimer"
                variant="destructive"
        ...
        >
            <td className="px-4 py-3">
                <input
                    ...
                    onChange={onToggle}
                    className="h-4 w-4 cursor-pointer accent-crimson"
                    aria-label={`Sélectionner ${tribune.name}`}
                />
            </td>
            <td className="px-4 py-3">
