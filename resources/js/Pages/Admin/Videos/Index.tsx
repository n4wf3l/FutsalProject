    ...
    return (
        <AdminLayout title="Vidéos">
            ...
            {filtered.length === 0 ? (
                <EmptyState
                    icon={Film}
                    title={search ? 'Aucune vidéo trouvée' : 'Aucune vidéo publiée'}
                    description={
                        search
                            ? 'Essaie une autre recherche ou vide le filtre.'
                            : 'Ajoute une première vidéo pour démarrer la galerie.'
                    }
                    action={
                        !search ? (
                            <Button asChild>
            ...
            <ConfirmDialog
                ...
                description="La vidéo et son image seront supprimées définitivement."
                confirmLabel="Supprimer"
                variant="destructive"
                onCancel={() => setToDelete(null)}
                onConfirm={() => {
                    if (!toDelete) return;
                    router.delete(`/videos/${toDelete.id}`, {
        ...
        >
            ...
            <td className="px-4 py-3">
                ...
                <a
                    ...
                    className="font-mono text-[10px] text-muted-foreground transition-colors hover:text-crimson"
                >
                    {video.url}
                </a>
            </td>
            <td className="hidden px-4 py-3 font-mono text-xs text-muted-foreground md:table-cell">
                {extractHost(video.url)}
            </td>
            <td className="px-4 py-3">
